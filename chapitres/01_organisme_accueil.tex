...
transition énergétique et environnementale. En 2025, il réunissait environ
293\,800~collaborateurs pour un chiffre d'affaires de 74,6~milliards d'euros
et un résultat net de 4,9~milliards d'euros~\cite{vinci_urd2024}.

Le groupe organise ses activités autour de trois grandes branches de métiers~\cite{wiki_vinci}.
VINCI~Concessions conçoit, finance et exploite des infrastructures
de transport, notamment à travers VINCI~Autoroutes et VINCI~Airports.
VINCI~Energies rassemble les services à l'énergie et au numérique.
VINCI~Construction et Eurovia couvrent le génie civil, le bâtiment et les
infrastructures routières. À ces branches s'ajoutent Cobra~IS, filiale
spécialisée dans les infrastructures énergétiques, et VINCI~Immobilier,
dédié à la promotion immobilière. C'est au sein de VINCI~Energies que
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    racine/.style={rectangle, rounded corners, draw=vinciblue, very thick,
      ...
      align=center},
    branche/.style={rectangle, rounded corners, draw=vinciblue, thick,
      fill=vinciblue!10, minimum width=2.3cm, minimum height=0.75cm, align=center},
    feuille/.style={rectangle, rounded corners, draw=gray, thick,
      fill=grisclair, minimum width=2.1cm, minimum height=0.65cm, align=center},
    lien/.style={-, draw=gray, thick}
  ]
    \node[racine] (vinci) {VINCI};

    \node[branche, below left=1.2cm and 4.8cm of vinci] (conc)
      {VINCI\\Concessions};
    \node[branche, below left=1.2cm and 1.2cm of vinci] (ve)
      {VINCI\\\textbf{Energies}};
    \node[branche, below right=1.2cm and 1.2cm of vinci] (vc)
      {VINCI\\Construction};
    \node[branche, below right=1.2cm and 3.8cm of vinci, minimum width=1.8cm] (cobra)
      {Cobra~IS};
    \node[branche, below right=1.2cm and 5.8cm of vinci, minimum width=2.0cm] (immo)
      {VINCI\\Immobilier};

    \draw[lien] (vinci) -- (conc);
    \draw[lien] (vinci) -- (ve);
    \draw[lien] (vinci) -- (vc);
    \draw[lien] (vinci) -- (cobra);
    \draw[lien] (vinci) -- (immo);

    \node[feuille, below=0.8cm of conc, xshift=-1.2cm, minimum width=1.9cm] (auto)
      {VINCI\\Autoroutes};
    \node[feuille, below=0.8cm of conc, xshift=1.2cm, minimum width=1.9cm] (airp)
      {VINCI\\Airports};
    \draw[lien] (conc) -- (auto);
    \draw[lien] (conc) -- (airp);

    \node[feuille, below=0.8cm of vc, xshift=-1.0cm, minimum width=1.9cm] (vcf)
      {VINCI Constr.\\France / Int.};
    \node[feuille, below=0.8cm of vc, xshift=1.0cm, minimum width=1.9cm] (eurovia)
      {Eurovia};
    \draw[lien] (vc) -- (vcf);
    \draw[lien] (vc) -- (eurovia);

    \begin{scope}[on background layer]
      \node[draw=vincired, thick, rounded corners, fit=(ve), inner sep=3pt] {};
    \end{scope}
  \end{tikzpicture}
...
mesure, de la conception-réalisation à l'exploitation-maintenance. En 2025,
la branche a réalisé un chiffre d'affaires de 21,6~milliards d'euros, avec
environ 109\,000~collaborateurs répartis dans 60~pays au sein de
2\,200~entreprises~\cite{ve_chiffres_cles}.

Sa principale caractéristique tient à son modèle décentralisé~:
VINCI~Energies fonctionne comme un réseau de près de 2\,200~entreprises à
...
\begin{table}[H]
  \centering
  \caption{Carte d'identité de VINCI~Energies (données 2025).}
  \label{tab:carte-ve}
  \begin{tabularx}{\textwidth}{>{\bfseries}p{4.2cm} X}
    ...
    Présence              & 60 pays, près de 2\,200 entreprises \\
    Effectifs (2025)      & environ 109\,000 collaborateurs \\
    Chiffre d'affaires (2025) & 21,6 milliards d'euros \\
...
Custom Applications, qui m'a accueilli.
...
d'utilisateurs clés et intègre les pratiques DevSecOps et de
numérique responsable dans ses projets. Elle représentait environ
...
chacun responsable d'un domaine applicatif~: Web \& Mobile
Applications, Collaborative Solutions, Payroll Solutions
et HRIS. J'ai été intégré au service Collaborative Solutions.
...
Microsoft~365, dans le respect des règles de gouvernance et de
...
interne VINCI~Energies Systèmes d'Information.}. L'équipe Group
Collaborative Solutions prend en charge les solutions à périmètre Groupe,
notamment l'intranet MyVIEW, le moteur de recherche et annuaire
Connect, et les espaces de travail collaboratifs. L'équipe
BU Collaborative Solutions se concentre sur les solutions à périmètre
Business Unit, comme DPF (Digital Project Files), ainsi que sur les
...
C'est au sein de l'équipe Group Collaborative Solutions que s'est
...
sécurité dès la conception (approche DevSecOps, analyse systématique
du code et des dépendances) et le numérique responsable
