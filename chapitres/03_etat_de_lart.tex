...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    font=\small,
    card/.style={rounded corners=4pt, draw=vinciblue, line width=0.9pt, fill=white,
      drop shadow={opacity=0.18,shadow xshift=1pt,shadow yshift=-1pt},
      align=center, text width=7cm, minimum height=1cm},
    layer/.style={rounded corners=3pt, draw=vinciblue!55, line width=0.7pt,
      fill=vinciblue!8, align=center, text width=6cm, minimum height=0.75cm},
    flow/.style={-{Stealth[length=3mm]}, line width=1.1pt, vinciblue},
    tag/.style={rounded corners=2pt, draw=vincired!55, fill=vincired!7,
      font=\scriptsize, text=grisfonce, align=center, inner sep=3pt},
    lbl/.style={font=\footnotesize\itshape, text=grisfonce, fill=white, inner sep=2pt}
  ]
    \node[card] (spa) {\textbf{Navigateur} — Application Angular (SPA)};
    \fill[vinciblue] ([xshift=7pt,yshift=-6.5pt]spa.north west) circle (1.3pt);
    \fill[vinciblue] ([xshift=13pt,yshift=-6.5pt]spa.north west) circle (1.3pt);
    \fill[vinciblue] ([xshift=19pt,yshift=-6.5pt]spa.north west) circle (1.3pt);
    \node[layer, below=1.5cm of spa] (ctrl) {Couche présentation — Contrôleurs};
    \node[layer, below=0.3cm of ctrl] (svc) {Couche métier — Services};
    \node[layer, below=0.3cm of svc] (repo) {Couche d'accès aux données — Repositories};
    \draw[flow] (ctrl) -- (svc);
    \draw[flow] (svc) -- (repo);
    \begin{scope}[on background layer]
      \node[draw=vinciblue, line width=1pt, rounded corners=5pt, fill=grisclair,
            fit=(ctrl)(svc)(repo), inner sep=12pt] (api) {};
    \end{scope}
    \node[anchor=south west, font=\bfseries\footnotesize, text=vinciblue]
          at ([xshift=3pt,yshift=2pt]api.north west) {API ASP.NET Core};
    \node[tag, right=0.7cm of ctrl] (t1) {DTO};
    \node[tag, right=0.7cm of repo] (t2) {Repository\\Unit of Work};
    \draw[vincired!55, dotted, line width=0.7pt] (ctrl.east) -- (t1.west);
    \draw[vincired!55, dotted, line width=0.7pt] (repo.east) -- (t2.west);
    \node[cylinder, shape border rotate=90, aspect=0.28, draw=vincired,
          line width=1pt, fill=vincired!8, minimum width=2.8cm, minimum height=1.2cm,
          align=center, below=1.6cm of api] (db) {\textbf{SQL Server}};
    \draw[flow] (spa) -- node[lbl,right]{Requêtes HTTP / REST (JSON)} (api.north);
    \draw[flow] (api.south) -- node[lbl,right]{Entity Framework Core (ORM)} (db);
  \end{tikzpicture}%
  }
...
constituent la grille de lecture qui servira, au chapitre~\ref{chap:csbag}, à
