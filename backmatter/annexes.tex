...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \label{fig:ann-admin}
  \end{subfigure}
  \caption{Écrans ajoutés dans \csbag v2 (1/2).}
  \label{fig:ann-ecrans}
\end{figure}

\begin{figure}[H]
  \centering
  \screenshot[0.85\textwidth]{csbag_historique_livraison.png}
  \caption{Historique de livraison d'un module~: date, document de mise en
    production, numéro de mise en production et fonctionnel associé.}
  \label{fig:ann-livraison}
\end{figure}
...
\section{Elium Sauvegarde~: flux de traitement et API}
\label{ann:elium}

Cette annexe complète la description de la mission Elium Sauvegarde présentée en
section~\ref{sec:elium}. La figure~\ref{fig:ann-elium-seq} détaille le
diagramme de séquence du traitement, de la lecture du référentiel SharePoint
jusqu'à la publication des pages. La figure~\ref{fig:ann-elium-postman}
présente les requêtes de l'API GraphQL d'Elium telles qu'utilisées pour le
développement et le diagnostic, via la collection Postman du service.

\begin{figure}[H]
  \centering
  \screenshot[\textwidth]{elium_sequence.png}
  \caption{Diagramme de séquence du traitement Elium Sauvegarde~: les cinq
    étapes, de la lecture du référentiel à la publication des pages SharePoint.}
  \label{fig:ann-elium-seq}
\end{figure}

\begin{figure}[H]
  \centering
  \screenshot[\textwidth]{elium_postman.png}
  \caption{Collection Postman de l'API Elium (GraphQL)~: requêtes utilisées
    pour le diagnostic et le développement.}
  \label{fig:ann-elium-postman}
\end{figure}
